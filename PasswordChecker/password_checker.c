...
static inline bool check_length(const char *password) {
    int length = strlen(password);
    bool meets_len_req = (length >= 10);
    return meets_len_req;
}

/* Returns true if LETTER is in the range [LOWER, UPPER], inclusive of the upper bound */
static inline bool check_range(char letter, char lower, char upper) {
...
otherwise */
static inline bool check_upper(const char *password) {
    while (*password != '\0') {
...
otherwise */
static inline bool check_lower(const char *password) {
    while (*password != '\0') {
        bool is_in_range = check_range(*password, 'a', 'z');
        if (is_in_range) {
            return true;
...
static inline bool check_number(const char *password) {
    while (*password != '\0') {
        if (isdigit(*password)) {
            return true;
        }
...
otherwise */
static inline bool check_name(const char *first_name, const char *last_name, const char
*password) {
